...
def main():
    ...
    try:
        ...
        remoteCommands = [f'tar -xf {remoteTarball} -C {tmp}', f'rm {remoteTarball}']
        ...
        remoteCommands += [
            f'cd {tmp}',
            f"export {' '.join(args.env)}",
            subprocess.list2cmdline(commandLine),
        ]

        return subprocess.call(ssh(' && '.join(remoteCommands)))
    finally:
        # Make sure the temporary directory is removed when we're done.
        subprocess.check_call(ssh(f'rm -r {tmp}'))
